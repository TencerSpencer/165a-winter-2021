...
class PageRange:
    ...
    def get_record(self, base_record_rid, query_columns):
        page_set_index = self.base_rids[base_record_rid][1]

        # start at the base page, get its schema too
        base_page_set = self.base_page_sets[page_set_index]
        tail_page = base_page_set.get_indirection(base_record_rid)

        # get only the base page's info
        if tail_page is None:
            return self.get_only_base_record(base_record_rid, page_set_index, query_columns)
        else:
            ...
            # obtain schema
            base_page_offset = base_page_set.rids[base_record_rid][2]
            ...
            tail_record_rid = self.base_indirections[self.base_rids[base_record_rid][2]]
            ...
            for i in range(self.num_columns):
                if query_columns[i] is not None:
                    if (base_page_schema >> i) == 1:
                        read_data.append(tail_page_set[i].read_record(tail_record_rid, query_columns, i))
                    else:
                        read_data.append(base_page_set[i].read_record(base_record_rid, query_columns, i))
    ...
    def update_record(self, base_rid, tail_rid, *columns):
        ...
        prev_tail_rid = self.get_indirection(base_rid)

        # get new schema for current update and base record
        base_offset = self.base_rids[base_rid][2]
        update_schema = 0
        ...
        new_schema = update_schema | self.base_schema_encodings[base_offset]

        # update schema for base record
        self.base_schema_encodings[base_offset] = new_schema

        # change base and tail record indirections accordingly
        if prev_tail_rid is None:
            prev_tail_rid = base_rid
        self.base_indirections[self.base_rids[base_rid][2]] = tail_rid
        ...
        if prev_tail_rid is not base_rid:
            new_columns = self.get_new_columns_for_new_tail(prev_tail_rid, columns)

        # write new tail with new schema and previous tails rid
        self.write_tail_record(tail_rid, new_schema, prev_tail_rid, new_columns)

    def get_new_columns_for_new_tail(self, prev_tail_rid, *columns):
        data = list(columns)
        tail_page_set_index, offset = self.tail_page_sets[prev_tail_rid][1:3]
    ...
    def write_tail_record(self, rid, schema, indirection, *columns):
        tail_page_set_index = self.num_tail_records / RECORDS_PER_PAGE
        tail_page_set = self.tail_page_sets[tail_page_set_index]
