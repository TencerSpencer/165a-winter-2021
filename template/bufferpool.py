...
class Bufferpool:
    def __init__(self):
        ...
        self.dirty_page_sets = set()  # set of (table_name, page_set_index, set_type) that are dirty and need to be written to disk before eviction
        self.pinned_pages = {}  # dict of (table_name, page_set_index, set_type) : pin_num indicating if the current RID is pinned. By default, this is zero
    ...
    def get_page_set(self, table_name, num_columns, disk, page_set_index, set_type, block_start_index):

        # if data is not in memory
        if self.pages_mem_mapping[(table_name, page_set_index, set_type)] is None:
            ...
            self.pages_mem_mapping[(table_name, page_set_index, set_type)] = data, num_columns

        else:  # pull data from current mem
            data = self.pages_mem_mapping[(table_name, page_set_index, set_type)]
    ...
    def __ensure_buffer_pool_can_fit_new_data(self, num_columns):
        ...
        while len(self.lru_enforcement) + num_columns + META_DATA_PAGES > MAX_PAGES_IN_BUFFER:
            table_name, page_set_index, set_type = self.lru_enforcement.pop_left()

            # page is currently pinned and we cannot evict it
            if self.pages_mem_mapping[(table_name, page_set_index, set_type)] is not None:
            ...
            # page can be evicted, remove    
                ...
                self.__evict_page_set(table_name, page_set_index)

    # evaluate if page is dirty then remove any traces
    def __evict_page_set(self, table_name, page_set_index):
        if self.__is_dirty(table_name, page_set_index):
            self.__write_to_disk(page_set_index, table_name)

        # remove entry from dictonary
        self.pages_mem_mapping.pop((table_name, page_set_index))

    # allocate new space for a page_set
    # assume meta data is packed
    def get_new_free_mem_space(self, table_name, page_set_index, num_columns, set_type):
        self.__ensure_buffer_pool_can_fit_new_data(num_columns)
    ...
    def pin_page_set(self, table_name, page_set_index, set_type):
        # start at zero and build up
        if self.pages_mem_mapping[(table_name, page_set_index, set_type)] is None:
            # add pair with 1 to indicate we just started pinning
            self.pages_mem_mapping[(table_name, page_set_index, set_type)] = 1
        else:
            # increase the amount of users using the page, for M3 safe keeping
            self.pages_mem_mapping[(table_name, page_set_index, set_type)] += 1

    def __is_dirty(self, table_name, page_set_index):
        return table_name, page_set_index in self.dirty_page_sets
    ...
    def pack_data(page_set, rids, timestamps, schema, indirections, indirection_types):
        ...
        for i in range((len(rids_data) // 4096) + 1):
            bytes = bytearray(rids_data[(PAGE_SIZE * i):(PAGE_SIZE * i) + PAGE_SIZE])
            ...
            bytes = bytearray(timestamps_data[(PAGE_SIZE * i):(PAGE_SIZE * i) + PAGE_SIZE])
            if len(bytes) != PAGE_SIZE:
                invalids = bytearray(PAGE_SIZE - len(bytes))
                for i in range(len(invalids)):
                    invalids[i] = -1
            ...
            bytes = bytearray(schema_data[(PAGE_SIZE * i):(PAGE_SIZE * i) + PAGE_SIZE])
            if len(bytes) != PAGE_SIZE:
                invalids = bytearray(PAGE_SIZE - len(bytes))
                for i in range(len(invalids)):
                    invalids[i] = -1
            ...
            bytes = bytearray(indirections_data[(PAGE_SIZE * i):(PAGE_SIZE * i) + PAGE_SIZE])
            if len(bytes) != PAGE_SIZE:
                invalids = bytearray(PAGE_SIZE - len(bytes))
                for i in range(len(invalids)):
                    invalids[i] = -1
            ...
            bytes = bytearray(indirection_types_data[(PAGE_SIZE * i):(PAGE_SIZE * i) + PAGE_SIZE])
            if len(bytes) != PAGE_SIZE:
                invalids = bytearray(PAGE_SIZE - len(bytes))
                for i in range(len(invalids)):
                    invalids[i] = -1
    ...
    def __write_to_disk(self, table_name, page_set_index, set_type):
        pass
